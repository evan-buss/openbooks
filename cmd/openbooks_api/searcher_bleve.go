...
func (b *BleveSearcher) Search(q string) SearchResultCollection {
	...
	return SearchResultCollection{
		...
		Results:  results,
		Duration: searchResults.Took.Milliseconds(),
	}
}

func (b *BleveSearcher) AddDocument(doc Book) {
...
func (b *BleveSearcher) AddDocuments(documents <-chan Book) time.Duration {
	batchSize := 1000
	count := 0
	startTime := time.Now()
